...
const skills = [
  { technology: "HTML", icon: "SiHtml5" },
  { technology: "CSS", icon: "SiCss3" },
  { technology: "Javascript", icon: "SiJavascript" },
  { technology: "Swift", icon: "SiSwift" },
  { technology: "Kotlin", icon: "SiKotlin" },
  { technology: "Wordpress", icon: "SiWordpress" },
  { technology: "Angular", icon: "SiAngular" },
  { technology: "NextJS", icon: "SiNextdotjs" },
  { technology: "ReactJS", icon: "SiReact" },
  { technology: "React Native", icon: "SiReact" },
  { technology: "Ionic Framework", icon: "SiIonic" },
  { technology: "Python", icon: "SiPython" },
  { technology: "AWS", icon: "FaAws" },
  { technology: "GCP", icon: "SiGooglecloud" },
];
...
const renderIcon = (icon: string) => {
  switch (icon) {
    ...
    case "SiSwift": return <SiSwift />;
    case "SiKotlin": return <SiKotlin />;
    case "SiWordpress": return <SiWordpress />;
    case "SiAngular": return <SiAngular />;
    case "SiNextdotjs": return <SiNextdotjs />;
    case "SiReact": return <SiReact />;
    case "SiIonic": return <SiIonic />;
    case "SiPython": return <SiPython />;
    case "FaAws": return <FaAws />;
    case "SiGooglecloud": return <SiGooglecloud />;
    default: return null;
  }
};

export default function CvContent({ locale }: { locale: string }) {
  const t = useTranslations("CV");
  const { toast } = useToast();
  const filteredCVs = useMemo(
    () => allCvs.filter((cv) => cv.lang === locale),
    [locale]
  );
  const categorizedCVs = useMemo(
    () =>
      filteredCVs.reduce((acc: any, cv: any) => {
        const event: any = cv.event;
        if (!acc[event]) acc[event] = [];
        acc[event].push(cv);
        acc[event].sort(
          (a: any, b: any) => new Date(b.from).getTime() - new Date(a.from).getTime()
        );
        return acc;
      }, {}),
    [filteredCVs]
    ...
    Object.entries(categorizedCVs)
      ...
      .map(([eKey, eValue]) => {
        ...
        (eValue as any).map((cv: any) => {
          string += [
            `Time range: ${format(new Date(cv.from), "MMM yyyy")}`,
  ...
  return (
    <Grid>
      ...
      <Grid.Right>
        <div className="flex justify-between">
          ...
          <div className="flex items-stretch gap-8 print:hidden">
            <Button className="self-center" size="icon" variant="ghost" onClick={handlePrint}>
              <Printer className="h-5 w-5" />
            </Button>
            <Button className="self-center" size="icon" variant="ghost" onClick={handleCopy}>
      ...
      <Grid.Left>
        <Text>{t("talk")}</Text>
        <Text variant="light">
          <Link href="mailto:iluisda.dev@icloud.com" target="_blank" rel="noopener">
            {"Email"}
          </Link>
        </Text>
      ...
      <Grid.Right>
        ...
        <div className={styles.prose}>
          <p className="mb-3">
            <strong className="capitalize">{t("keybirthday")}</strong>: {t("birthday")}
            <br />
            <strong className="capitalize">{t("keycitizenship")}</strong>: {t("citizenship")} {getFlagEmoji("VE")}
            <br />
            <strong className="capitalize">{t("keylanguages")}</strong>: {t("languages")}
            <br />
            <strong className="capitalize">{t("keylocation")}</strong>: {t("location")} 📍 {getFlagEmoji("BR")}
            <br />
            <strong className="capitalize">{t("keyhobbies")}</strong>: {t("hobbies")} &#127947; &#128692; &#128745; &#129406; &#128054; 🎧
            <br />
          </p>
        </div>
      </Grid.Right>
